...
@WebServlet("/Fetchpan")
public class Fetchpan extends  HttpServlet{
  ...
  response) throws ServletException, IOException{
     ...
     long Acount_number = Long.parseLong(n);
	  try{
	      ...
	      PreparedStatement ps = con.prepareStatement("select pan from uploaddocument where acount=?");
	     
	      ps.setLong(1,Acount_number);
	      ResultSet rs = ps.executeQuery();
	     
	      if(rs.next()){
	          Blob photo = rs.getBlob("pan");
