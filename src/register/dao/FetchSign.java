...
@WebServlet("/FetchSign")
public class FetchSign extends  HttpServlet{
  ...
  response) throws ServletException, IOException{
     ...
     Long Acount_number = Long.parseLong(n);
	  try{
	      ...
	      PreparedStatement ps = con.prepareStatement("select sign from uploaddocument where acount=?");
	     
	      ps.setLong(1,Acount_number);
	      ResultSet rs = ps.executeQuery();
	     
	      if(rs.next()){
	          Blob photo = rs.getBlob("sign");
